...
import { CLUB_PLAYERS, PLAYERS, SEASONS } from '../../../constants/endpoints';
...
const EditPlayer = ({navigation}:any) => {
    const [player, setPlayer] = useState({
     ...
     const {clubName , clubId, playerId, seasonId} =  useRoute<any>().params
     ...
     const fetchPlayer = async () => {
        try {
            const playerRef = await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(clubId).collection(PLAYERS).doc(playerId).get()
    ...
    //fetch player 
     ...
     const onEditPlayer = async () => {
        try {
            ...
            setLoading(true)
           //edit player
            await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(clubId).collection(PLAYERS).doc(playerId).update({
                name: player.name,
                club: clubName,
            ...
            navigation.navigate("ClubDetailsScreen", { clubId: clubId , seasonId: seasonId})
            //navigation.navigate("")
