...
const PlayerDetails = ({ navigation }: any) => {
    ...
    const { playerId, clubId, seasonId } = useRoute<any>().params
    ...
    const getClubs = async () => {
        try {
            setLoading(true)
            setClubs([])
            const clubs = await firestore().collection(SEASONS).doc(seasonId).collection(CLUBS).get();
    ...
    const getClubData = async () => {
        const clubDoc = await firestore().collection(SEASONS).doc(seasonId).collection(CLUBS).doc(clubId).get();
         
        const playerDoc = await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(clubId).collection(PLAYERS).doc(playerId).get();
    ...
    const onFinishTransfer = async () => {
        try {
            ...
            await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(clubId).collection(PLAYERS).doc(playerId).delete();
            
            //add to new club
            await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(selectedClub.id).collection(PLAYERS).doc(playerId).set(details);
            showMessage({
