...
const Players= ({ navigation }: any) => {
    ...
    const { clubId,clubName, seasonId } = useRoute<any>().params;
    ...
    const getPlayers = async () => {
        try {
            setLoading(true)
            setClubs([])
            const clubs = await firestore().collection(SEASONS).doc(seasonId).collection(CLUB_PLAYERS).doc(clubId).collection(PLAYERS).get();
