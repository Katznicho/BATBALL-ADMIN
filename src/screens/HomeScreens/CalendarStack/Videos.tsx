...
import { USER_POSTS, USER_STATUS, VIDEOS } from '../../../constants/endpoints';
...
const Videos = () => {
    ...
    const getLatestStatusesForUsers = async () => {
        try {
            const statuses = await firestore().collection(VIDEOS).get(); // Assuming you have a "users" collection
        ...
        <SafeAreaView style={[generalstyles.container]}>
            {loading ? (
                ...
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{ color: theme.colors.primary, fontSize: 20, fontWeight: "bold" }}>No Videos Yet</Text>
                </View>
            ) :
            (<FlatList
                data={status}
